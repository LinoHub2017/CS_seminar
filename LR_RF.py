...
df = pd.get_dummies(df)
df = pd.get_dummies(df, columns=['holidays','hour','week','month','temp'])
...
"""
poly_transformer = PolynomialFeatures(degree=100, interaction_only=True, include_bias=False)
df_onehot_poly = poly_transformer.fit_transform(df)
#lr = Ridge()
eval_on_features(df_onehot_poly, y, Ridge())
